...
- PC POST /api/leads는 유효성·중복 검사를 거쳐 기존 D1 `leads`에 저장. 체크박스 동의와 그 기록은 새 접수에 사용하지 않는다. 메일 알림은 FormSubmit을 통해 박성호의 운영 메일로 보낸다. `LEAD_NOTIFY_EMAIL` 비밀 환경값과 수신 메일 인증은 설정·확인했다. 공식 사이트 시험 접수 #4의 저장과 메일 수신을 확인했다.
