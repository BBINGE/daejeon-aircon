...
2026-09-21 운영 기준. 과거 DB 운영 원문은 HISTORY_LEGACY.md에 보존. PC 접수는 공식 사이트에 게시했고, 시험 접수의 DB 저장과 운영 메일 수신을 확인했다.

## 상담

- 010-9183-2200 전화·문자로 대표 직접 수신. 사진도 같은 번호.
- 모바일은 기존 전화·문자 직통을 유지. PC 상담 신청은 연락처 필수, 작업·지역 선택 입력으로 박성호가 먼저 확인하여 대표에게 전달한다.
- PC POST /api/leads는 유효성·동의·중복 검사를 거쳐 기존 D1 `leads`에 저장. 동의 문구는 상담 전화 목적과 개인정보 안내 링크만 표시하고, 새 문구 버전과 시각을 기록한다. 메일 알림은 FormSubmit을 통해 박성호의 운영 메일로 보낸다. `LEAD_NOTIFY_EMAIL` 비밀 환경값과 수신 메일 인증은 설정·확인했다. 공식 사이트 시험 접수 #4의 저장과 메일 수신을 확인했다.
