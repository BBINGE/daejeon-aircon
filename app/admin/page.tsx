...
const statusLabels: Record<string, string> = { new: "신규", contacted: "연락 완료", quoting: "견적 진행", won: "계약", completed: "작업 완료", hold: "보류" };
...
export default function AdminPage() {
  ...
  if (!authenticated) return <main className="admin-login"><form onSubmit={load}><span>LEAD DESK</span><h1>에어컨 문의 관리</h1><p>관리 비밀번호를 입력하면 접수 목록을 확인할 수 있습니다.</p><label>관리 비밀번호<input type="password" value={key} onChange={e => setKey(e.target.value)} required autoFocus /></label>{error && <div className="admin-error">{error}</div>}<button disabled={loading}>{loading ? "확인 중" : "관리 화면 열기"}</button><a href="/">고객 화면으로 돌아가기</a></form></main>;
