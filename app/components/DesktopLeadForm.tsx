...
export default function DesktopLeadForm() {
  ...
  return <div className="desktop-lead" id="pc-inquiry">
    <div className="desktop-lead-copy"><span>PC 상담 신청</span><h3>번호만 남기면<br />상담으로 이어드립니다.</h3><p>박성호가 신청을 확인해 김대곤 대표에게 전달합니다. 대표가 연락드려 작업 가능 여부를 상담합니다.</p><a href="tel:01091832200">바로 통화하고 싶다면 010-9183-2200</a></div>
    {state === "done" || state === "done-needs-call" ? <div className="desktop-lead-done" role="status"><strong>상담 신청을 받았습니다.</strong><p>{state === "done-needs-call" ? "접수 알림이 지연되고 있어요. 빠른 상담을 원하시면 010-9183-2200으로 직접 전화해주세요." : "박성호가 확인해 김대곤 대표에게 전달하겠습니다. 급한 문의는 010-9183-2200으로 전화해주세요."}</p></div> : <form className="desktop-lead-form" onSubmit={submit}>
      ...
      <button type="submit" disabled={state === "sending"}>{state === "sending" ? "신청 중…" : "전화 상담 신청하기"}</button><p className="desktop-lead-foot">신청만으로 예약·견적이 확정되지 않습니다.</p>
    </form>}
  </div>;
}
